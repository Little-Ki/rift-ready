<template>
    <div class="view-content">
        <title-view value="自动选用" :buttons="['reload', 'plus']" @button="onButton">
            <switcher :value="config.data.autoPick.enable" @switch="switchEnable" />
        </title-view>
        <vue-scrollbar width="100%" height="calc(100% - 48px)" background="white" radius="0.25rem" scroll-y>
            <transition-group name="list">
                <div class="list-empty" v-if="config.data.autoPick.championIds.length == 0" key="none">什么也没有</div>
                <li class="list-item" v-else v-for="(it, index) of config.data.autoPick.championIds" :key="it">
...
const isEnable = computed(() => config.data.autoPick.enable);
...
const createBan = (hero: Hero) => {
    const tempList = [...config.data.autoPick.championIds]
...
const removeBan = (id: number) => {
    const temp = [...config.data.autoPick.championIds]
...
const switchEnable = (value: boolean) => {
    Api.config.set({
        ...
        autoPick: {
            enable: value,
            championIds: config.data.autoPick.championIds
        }
    }).then(({ data }) => {
        config.patch(data.data)
    })
